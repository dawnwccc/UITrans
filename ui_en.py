...
def convert_android_page_to_harmony(android_xml):
    if not android_xml:
        gr.Warning("Error: Please input the Android XML page code first!")
        return ""
    # 初始化llm
    tqdm_progress = tqdm(total=100, desc="Translation progress")
    llm_client = get_llm_client()
    code_monkey_agent = CodeMonkeyAgent(llm_client=llm_client)
...
def parse_android_project(android_zip):
    if not zipfile.is_zipfile(android_zip):
        gr.Warning("Error: Invalid zip file!")
        return False, None
    ...
    with zipfile.ZipFile(android_zip, 'r') as zip_ref:
        ...
        if len(root_folders) == 1:
            ...
            gr.Warning("Error: The .zip file does not contain a single root directory folder!")
            return False, None
...
def convert_android_project_to_harmony(android_project_path_id):
    if not android_project_path_id:
        gr.Warning("Error: Please upload the Android project Zip archive and wait for the parsing to complete!")
        return "Please upload the Android project Zip archive first!", None
    project_progress_bar = tqdm(total=100, desc="Translation progress")
    ...
    return "Translation completed. Click on the Zip archive to download.", zipped_harmony_project_path


def handle_download_harmony_project(harmony_project_path: str):
    if not harmony_project_path:
        gr.Warning("Error: Please translate the Android project first!")
        return ""
...
with gr.Blocks(css=global_css) as app:
    html_content = f"""
        <div style="display: flex; align-items: center; justify-content: center; height: 28vh; padding: 10px; margin: 0 auto; overflow: hidden;">
    <div style="margin-right: 10px;">
        <img src="https://hyj-first-1306572465.cos.ap-nanjing.myqcloud.com/uitrans_bg.png" alt="Logo Image" style="width: 200px; height: 200px;">
    </div>
    <div>
        <h1 style="font-size: 64px; color: black;margin-left: 150px;">UITrans</h1>
        <p style="font-size: 16px; color: gray;">A smart translation tool that supports converting Android UI to HarmonyOS ArkUI</p>
    </div>
</div>
        """
    ...
    #     </div>
    # """)
    with gr.Tab("Component"):
        gr.Markdown("## Component translation")
        gr.Markdown("Please paste the Android XML component code into the input box below, then click the \"Confirm\" button to translate it into HarmonyOS code.")
        android_xml_input = gr.TextArea(label="Input Android XML component", lines=10, max_lines=100)
        component_example_image = gr.Image(visible=True, label="Diagram/Schematic illustration")
        convert_button = gr.Button("Confirm")
        harmonyos_output = gr.TextArea(label="Output HarmonyOS code", lines=10, max_lines=100)
        ...
        component_examples = gr.Examples(
            ...
            inputs=[gr.Text(visible=False, label="Component name"), android_xml_input, component_example_image],
        )
    with gr.Tab("Page"):
        gr.Markdown("## Page translation")
        gr.Markdown("Please paste the Android XML page code into the input box below, then click the \"Confirm\" button to translate it into HarmonyOS code.")
        android_xml_input = gr.TextArea(label="Input Android XML page", lines=20, max_lines=100)
        page_example_image = gr.Image(visible=True, height=300, label="Diagram/Schematic illustration")
        convert_button = gr.Button("Confirm")
        harmonyos_output = gr.TextArea(label="Output HarmonyOS code", lines=10, max_lines=100)
        ...
        page_examples = gr.Examples(
            ...
            inputs=[gr.Text(visible=False, label="Page name"), android_xml_input, page_example_image],
        )
    with gr.Tab("Project"):
        gr.Markdown("## Project translation")
        gr.Markdown("Please upload the Zip archive of the Android project in the file upload box below, then click the \"Confirm\" button to translate it into HarmonyOS code.")
        android_xml_zip_input = gr.File(label="Upload Android project archive")
        message_output = gr.Text(label="Translation progress", interactive=False, lines=1, max_lines=1, autoscroll=True)
        convert_button = gr.Button("Start translation", interactive=True)
        project_progress_bar = gr.Progress(track_tqdm=True)
        android_project_path_id_text = gr.Text("", visible=False, label="Android project path")
        harmony_output = gr.File(label="Download HarmonyOS project Zip archive")
        ...
        def handle_parse_android_project(_android_xml_zip):
            ...
            if android_xml_parsed_flag:
                ...
                return "Parsing successful, click the confirm button to start translation.", project_path_id
            else:
                return "Parsing failed, please upload the correct Android project Zip archive.", ""
        ...
        project_examples = gr.Examples(
            ...
            inputs=[gr.Text(visible=False, label="Project name"), android_xml_zip_input],
            run_on_click=True,
